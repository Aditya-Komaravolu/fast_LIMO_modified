...
void lidar_callback(const sensor_msgs::PointCloud2::ConstPtr& msg){
    ...
    // Publish ground plane if not already published
    if (loc.is_calibrated() && !ground_plane_published) {
        ROS_INFO("IMU is calibrated, creating ground plane");
        
        // Get position and rotation from the localizer
        Eigen::Vector3f initial_position = loc.getInitialPosition();
        Eigen::Matrix3f initial_rotation = loc.getInitialRotation();
        
        // Create and publish ground plane marker
        visualization_msgs::Marker ground_plane = visualize_limo::getGroundPlaneMarker(
            initial_position, initial_rotation);
        
        ground_plane.header.frame_id = world_frame;
        ground_plane_pub.publish(ground_plane);
        
        ROS_INFO("Published ground plane at [%f, %f, %f]", 
                 ground_plane.pose.position.x,
                 ground_plane.pose.position.y,
                 ground_plane.pose.position.z);
...
int main(int argc, char** argv) {
    ...
    // Add this line with the other publishers
    ...
    if (config.offline_mode){
        ...
        visualization_msgs::Marker static_ground;
        static_ground.header.frame_id = "map";
        static_ground.header.stamp = ros::Time::now();
        static_ground.ns = "static_ground";
        static_ground.id = 0;
        static_ground.type = visualization_msgs::Marker::CUBE;
        static_ground.action = visualization_msgs::Marker::ADD;
        static_ground.pose.position.x = 0;
        static_ground.pose.position.y = 0;
        static_ground.pose.position.z = -0.05;
        static_ground.pose.orientation.w = 1.0;
        static_ground.scale.x = 50.0;
        static_ground.scale.y = 50.0;
        static_ground.scale.z = 0.02;
        static_ground.color.g = 0.5;
        static_ground.color.b = 1.0;
        static_ground.color.a = 0.5;
        static_ground.lifetime = ros::Duration();

        // Create a dedicated publisher for the static ground plane
        static_ground_pub.publish(static_ground);
        ROS_INFO("Published static ground plane");
